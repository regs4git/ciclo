...
- **v4** — corrigido o bug que impedia o popup de fase, o onboarding do nome e o aviso de nova versão de alguma vez desaparecerem: regras de CSS próprias (`display: flex` incondicional) anulavam o atributo `hidden` definido pelo JavaScript. Adicionada regra `[hidden]{display:none!important}` global. Enter no campo do nome do onboarding também guarda agora.
